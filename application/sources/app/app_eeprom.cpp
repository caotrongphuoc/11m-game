...
#include "game/game_eleven_meter/em_game_types.h"
...
#define EM_GAME_SCORE_CHECKSUM_SIZE (sizeof(uint32_t) + sizeof(em_game_score_record_t))
#define EM_GAME_SCORE_EEPROM_SIZE (9)

static_assert(sizeof(em_game_score_eeprom_t) == EM_GAME_SCORE_EEPROM_SIZE, "em_game score EEPROM record must be 9 bytes");
...
static void em_game_eeprom_update_checksum(em_game_score_eeprom_t* eeprom_data)
{
	eeprom_data->magic_number = EM_GAME_EEPROM_MAGIC_NUMBER;
	eeprom_data->check_sum = em_game_eeprom_checksum((const uint8_t*)eeprom_data, EM_GAME_SCORE_CHECKSUM_SIZE);
}
...
{
	data->best_goals = 0;
	data->best_difficulty = EM_GAME_DIFFICULTY_EASY;
	data->reserved[0] = 0;
	data->reserved[1] = 0;
}
...
bool load_score_record(em_game_score_record_t* data)
{
	...
	if ((ret == EEPROM_DRIVER_OK) &&
	    em_game_eeprom_is_valid(&eeprom_data))
	{
