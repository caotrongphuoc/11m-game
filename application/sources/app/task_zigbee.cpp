...
int zb_znp::zigbee_message_handler(zigbee_msg_t& zigbee_msg)
{
	...
	case AF_INCOMING_MSG:
	{
		...
		{
			ZB_DBG("Message was processed\n");
		}
		break;
		case ZCL_PROC_INVALID:
		{
			ZB_DBG("Format or parameter was wrong\n");
		}
		break;
		case ZCL_PROC_EP_NOT_FOUND:
		{
			ZB_DBG("Endpoint descriptor not found\n");
		}
		break;
		case ZCL_PROC_NOT_OPERATIONAL:
		{
			ZB_DBG("Can't respond to this command\n");
		}
		break;
		case ZCL_PROC_INTERPAN_FOUNDATION_CMD:
		{
			ZB_DBG("INTER-PAN and Foundation Command (not allowed)\n");
		}
		break;
		case ZCL_PROC_NOT_SECURE:
		{
			ZB_DBG("Security was required but the message is not secure\n");
		}
		break;
		case ZCL_PROC_MANUFACTURER_SPECIFIC:
		{
			ZB_DBG("Manufacturer Specific command - not handled\n");
		}
		break;
		case ZCL_PROC_MANUFACTURER_SPECIFIC_DR:
		{
			ZB_DBG("Manufacturer Specific command - not handled, but default response sent\n");
		}
		break;
		case ZCL_PROC_NOT_HANDLED:
		{
			ZB_DBG("No default response was sent and the message was not handled\n");
		}
		break;
		case ZCL_PROC_NOT_HANDLED_DR:
		{
			ZB_DBG("default response was sent and the message was not handled\n");
		}
		break;
		default:
		{
...
void task_zigbee(ak_msg_t* msg)
{
	...
	case AC_ZIGBEE_ZCL_CMD_HANDLER:
	{
		...
		case ZCL_CLUSTER_ID_MS_RELATIVE_HUMIDITY:
		{
			...
			(void)retHum;
			// For example, retHum = 6789 represents 67.89% relative humidity.
		...
		case ZCL_CLUSTER_ID_MS_TEMPERATURE_MEASUREMENT:
		{
			...
			(void)retTemp;
			// For example, retTemp = 2723 represents 27.23 degrees Celsius.
